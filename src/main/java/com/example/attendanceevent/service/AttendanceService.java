...
public class AttendanceService {
    ...
    public String markAttendance(Long userId) {
        ...
        // 2. 실행: 출석 안했다면? 데이터 넣기
        Attendance attendance = saveAttendance(userId);

        // 3. 후속작업: 연속 일수 계산
        int continuousDays = getContinuousDays(userId);

        // 4. 보상
        return processReward(userId, continuousDays, attendance.getId());
    }
    ...
    private int getContinuousDays(Long userId) {
        return attendanceMapper.countContinuousAttendance(userId);
    }


    private @NonNull Attendance saveAttendance(Long userId) { //NonNull은 무슨의미지 여기서?
